  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-400 mt-1">Run decisions for all 200 users simultaneously</p>
      </div>

      <div className="card flex flex-wrap items-end gap-4">
        <div>
          <label className="label">Ad</label>
          <select className="select-input mt-1" value={adId} onChange={(e) => setAdId(e.target.value)}>
            {ads.map((a) => <option key={a.id} value={a.id}>{a.id} - {a.category}</option>)}
          </select>
        </div>
        <div>
          <label className="label">Time</label>
          <select className="select-input mt-1" value={time} onChange={(e) => setTime(e.target.value)}>
            {['morning','afternoon','evening','latenight'].map((t) => <option key={t}>{t}</option>)}
          </select>
        </div>
        <div>
          <label className="label">Season</label>
          <select className="select-input mt-1" value={season} onChange={(e) => setSeason(e.target.value)}>
            {['Spring','Summer','Fall','Winter'].map((s) => <option key={s}>{s}</option>)}
          </select>
        </div>
        <div>
          <label className="label">Ads shown: {adsShown}</label>
          <input type="range" min={0} max={5} value={adsShown} onChange={(e) => setAdsShown(Number(e.target.value))} className="mt-2 w-24" />
        </div>
        <div>
          <label className="label">Fatigue: {fatigue.toFixed(2)}</label>
          <input type="range" min={0} max={1} step={0.05} value={fatigue} onChange={(e) => setFatigue(Number(e.target.value))} className="mt-2 w-24" />
        </div>
      ...
      {rows.length > 0 && (
        <>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="card">
              <h2 className="font-semibold mb-3">Decision Distribution</h2>
              <ResponsiveContainer width="100%" height={220}>
                <PieChart>
                  <Pie data={pieData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`} labelLine={false}>
                    {pieData.map((entry) => <Cell key={entry.name} fill={DECISION_COLORS[entry.name] ?? '#6b7280'} />)}
                  </Pie>
            ...
            <div className="card">
              <h2 className="font-semibold mb-3">Counts</h2>
          ...
          <div className="card">
            <div className="flex items-center gap-2 mb-3">
              <h2 className="font-semibold">Per-user Results</h2>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="text-left text-gray-500 border-b border-gray-800">
                  <th className="pb-2">User</th><th className="pb-2">Age</th><th className="pb-2">Decision</th><th className="pb-2">Score</th>
                </tr></thead>
                <tbody>
                  {filtered.slice(0, 50).map((r) => (
                    <tr key={r.user_id} className="border-b border-gray-900">
                      <td className="py-1.5">{r.user_name}</td>
                      <td className="py-1.5 text-gray-400">{r.age_group}</td>
                      <td className="py-1.5"><DecisionBadge decision={r.decision} size="sm" /></td>
                      <td className="py-1.5 font-mono text-gray-400">{r.combined_score.toFixed(3)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              {filtered.length > 50 && <p className="text-xs text-gray-500 mt-2">Showing 50 of {filtered.length}</p>}
            </div>
          </div>
        </>
      )}
